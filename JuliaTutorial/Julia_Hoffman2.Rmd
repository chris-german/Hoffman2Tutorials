...
On a cluster, each simulation can be submitted in a jobarray, which can be spread across different compute nodes. The syntax depends on the scheduling system. On UCLA's Hoffman2 cluster, `qsub` is used. In [`clusterSim.sh`](./clusterSim.sh), we submit sample sizes n (100, 200, ..., 500) and generative models given as a command-line argument. (standard normal, T distribution with 5 degree of freedom, and T distribution with 1 degree of freedom) and submit the jobs using `qsub`. 
