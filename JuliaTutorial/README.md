...
2. If you are using Julia from the system-wide installation via `module load Julia` (or downloaded the prebuilt Julia by yourself from Julia's download web site), it is recommended to on the Intel nodes, as required by the prebuilt Julia. To do so, add the option `-l arch=intel*` to `qsub`, or `-l arch=intel\*` to qrsh. (The escape character `\` is necessary in `qrsh` since it is issued from the command prompt.)
